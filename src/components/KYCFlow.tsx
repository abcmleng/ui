...
export const KYCFlow: React.FC = () => {
  ...
  useEffect(() => {
    if (currentStep === 3) { // After document_type step (index 2), before document-front (index 3)
      // Determine scanner type based on metadata
      if (selectedCountryCode && selectedDocumentType) {
        ...
        if (meta) {
          ...
          if (barcode === 'TD1' || barcode === 'TD2' || barcode === 'TD3' || barcode === 'TD3 B' || barcode === 'TD2 B') {
            setScannerType('mrz');
          } else if (barcode === 'PDF417 F' || barcode === 'PDF417 B'|| barcode === 'QR B' || barcode === 'ITF B') {
  ...
  const handleMRZScan = async (mrzData: string) => {
    ...
    // Submit final verification after scan
    ...
    try {
      ...
      // Show the error message as is without prefix
  ...
  const nextStep = () => {
    setCurrentStep((prev) => {
      ...
      // Skip 'document-back' step if selected document type is passport ('PP')
      if (
        STEPS[prev] === 'document-front' &&
        selectedDocumentType?.toLowerCase() === 'pp'
      ) {
        next = prev + 2; // Skip 'document-back'
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* ProgressIndicator removed to hide status */}
      {renderCurrentStep()}
    </div>
  );
};
